...
1. append =$HOME/.config/emacs/bin= to path (already set up if using zsh)
2. =doom sync= will install and compile all core essentials
3. =doom doctor= will list all dependencies needed for current plugins. Install all that are needed for you
** To install NeoViM:
