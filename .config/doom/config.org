...
#+END_SRC
The modeline should not be showing information that is assumed to be true, ie UTF-8
#+begin_src emacs-lisp
...
* DESKTOP FILE
We now have an easy way to start the daemon, but no programs will use the client. The solution? .desktop files!
...
:END:
This makes Emacs go faster for some reason...
...
Specify a :branch to install a package from a particular branch or tag. This is required for some packages whose default branch isn’t ’master’ (which our package manager can’t deal with; see raxod502/straight.elsrc  emacs-lisp
