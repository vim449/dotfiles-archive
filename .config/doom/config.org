...
+ Compressing the stages of a project --- a more centralized workflow can progress
  with greater ease
...
+ Recursive editing
+ Completely introspect-able, with pervasive docstrings
...
user to pick between common desirable behaviors. To me this is now like /giving
a man a fish/.
...
manually set the value for /every single instance/. Ugh, [[https://github.com/microsoft/vscode/issues/93153][what]] [[https://github.com/microsoft/vscode/issues/93628][a]] [[https://github.com/microsoft/vscode/issues/5595][pain]].
...
program which modifies the behavior of the editor to suit your liking*.
...
Want to tweak the behavior? Just re-evaluate your new version --- it's a
...
#+end_src
*** Windows
Let's split in the direction that makes sense by default, alright?
#+begin_src emacs-lisp
...
#+end_src
**** Language Support
Doom is pretty smart when it comes to language modules, so there is not tons of a performance hit until you open that file type. As such, just enable every language you know.
...
#+end_src
In additions to these fonts, I use Merriweather for certain modes, and Cantarell as a serifed proportional font for all of the aesthetic modes like =write-room=.
...
#+end_src
*** Splash Screen
Emacs is capable of rendering an image as a splash screen, or doing arbitrary computation to create a screen. [[https://github.com/MarioRicalde][@MarioRicalde]] created some great images, and now I just need to make some code to display the correct number of symbols based on the frame size...
#+begin_src emacs-lisp
...
Hey, let’s get the weather in here while we’re at it. Unfortunately this seems slightly unmaintained (few open bug-fix PRs) so let’s roll our own version.
#+begin_src emacs-lisp
...
When I use dired, I occasionally am too lazy to open a new buffer to see images, so...
#+begin_src emacs-lisp
(package! peep-dired)
#+end_src
*** Features
...
** Language Packages
*** LaTeX
Not sure what this is, exactly, but I like what it does.
#+begin_src emacs-lisp
...
I don't trust AucTeX very much at all, but I trust this one more...
#+begin_src emacs-lisp
...
~org-md~ is a goody-two-shoes, it just uses HTML for all these non-standardized
...
Org-roam is nice by itself, but there are so many /extra/ nice packages which integrate
with it.
#+begin_src emacs-lisp
...
#+end_src
*** Systemd
Not a super often use-case, but nice to have when it /is/ needed.
#+begin_src emacs-lisp
...
#+end_src
*** VimL
I know, I know, this sounds useless, but its actually /somewhat/ useful to have when I need to edit my neovim config, which uses 7 different coding languages.
#+begin_src emacs-lisp
(package! vimrc-mode)
#+end_src
* Package Config
** Abbrev Mode
Thanks to some stack overflow wizards, I can have all of my abbreviations in the same document.
#+begin_src emacs-lisp
(use-package abbrev
  :init
...
#+end_src
** Avy
Avy uses the QWERTY home-row for position jumping, but I would rather not use the g key..
#+begin_src emacs-lisp
(after! avy
...
** Centaur-tabs
Before this code will work, you have to un-comment =tabs= in the init.el section. Also, unicode for some reason causes lag, so let's change it.
#+begin_src emacs-lisp
(after! centaur-tabs
  (centaur-tabs-mode -1)
...
** Ptemplate
#+begin_src emacs-lisp
;; (setq! ptemplate-project-template-dirs '('~/'))
#+end_src
#+begin_src emacs-lisp
...
#+end_src
**** Symbols
Most of what I do in org involves making giant trees, and collapsing what I don't need to see, so I'm going to go ahead and change the collapsed items symbol to ▾. Also, while we're at it, why not add all the symbols we can ever want, and  un-comment the ones we use?
#+begin_src emacs-lisp
...
+ Caption labels could do with being emphasized slightly more
...
#+begin_src LaTeX
% args = #1 Name, #2 Color, #3 Ding, #4 Label
...
a cannibalized version of ~org-babel-tangle~ which just collects the file names of
...
src_elisp{:serif}, src_elisp{:sans}, src_elisp{:mono}, and src_elisp{:math}.
...
#+end_src
* Applications
While Emacs is mostly used for my programming needs, I like to use emacs for many other things as well. These configuration changes don't fit in well with the rest of the config, so I shove them here.
** Mu4e
...
** Elfeed
While it may seem otherwise, RSS feeds are still a perfectly valid and useful thing. =Elfeed= is an emacs package, with a corresponding doom module, that acts as an RSS feed reader. There's plenty of things to configure here, but most of it comes from  [[github:fuxialexander/doom-emacs-private-xfu/tree/master/master/modules/app/rss][fuxialexander]].
*** Keybindings
#+begin_src emacs-lisp
...
#+end_src
** Irc
The freenode debacle drew me into the concept of IRC, and I have found that it's actually pretty nice once you get used to a messaging system that doesn't store anything for you. For the most part, I've just used the semi-official kiwi client for messaging, so here's my hack-job, mostly curtesy Teco (as most of this is), to set up Circe.

First, we need to set up the whole authentication hassle, so let's make things easy and put all of the server information in =.authinfo.gpg= (I store mine in a more XDG compliant place, but that's besides the point).
Here's what an example authinfo would look like:
